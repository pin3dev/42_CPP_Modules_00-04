...
/*   Updated: 2024/01/22 18:54:34 by pin3dev          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	std::cout << BKGRAY "Default constructor called" RESET << std::endl;
}
// Copy constructor
ClapTrap::ClapTrap(const ClapTrap &src) : _name(src._name), _hit(src._hit), _energy(src._energy), _damage(src._damage)
{
    std::cout << BKGRAY "Copy constructor called to " << this->get_name() + RESET  << std::endl;
	//*this = src;
}
...
{
	std::cout << BKGRAY "Constructor called to " << this->get_name() + RESET  << std::endl;
}
// Destructor
ClapTrap::~ClapTrap()
{
	std::cout << BKGRAY "Destructor called to " << this->get_name() + RESET << std::endl;
}
...
ClapTrap &ClapTrap::operator=(const ClapTrap &src)
{
	...
	std::cout << BKGRAY "Copy assignment operator called to " << this->get_name() + RESET  << std::endl;
	return (*this);
}
...
// Checks if the currently instance have enought value of _hit or _energy, returning a refer status
...
// Prints error mensage about the passed error type and return a KO status
...
// Prints full status of instance and a status to the action that called this funct (attack, TakeDamage, beRepaired)
